...
def add_train_args(parser):
    ...
    runtime.add_argument('--train_batch_size', type=int, default=32,
    ...
    files.add_argument('--model_name', type=str, default='my_model1',
                       help='Unique model identifier (.mdl, .txt, .checkpoint)')
    files.add_argument('--data_dir', type=str, default='../data/',
                       help='Directory of training/validation data')
    files.add_argument('--train_src', nargs='+', type=str, default=['train1.token.code'],
    ...
    files.add_argument('--train_tgt', nargs='+', type=str, default=['train.token.nl'],
                       help='Preprocessed train target file')
    files.add_argument('--dev_src', nargs='+', type=str, default=['valid1.token.code'],
    ...
    files.add_argument('--train_init_summ', type='bool', default=['train1.init.summ'],
                       help='Use structural information of code')
    ...
    if not args.only_test:
        ...
        if num_dataset > 1:
            ...
            if len(args.train_init_summ) == 1:
                args.train_guid = args.train_init_summ * num_dataset
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tag, train_tgt, train_guid, dataset_name, train_init_summ in \
                zip(args.train_src_files, args.train_src_tag_files,
                    args.train_tgt_files, args.train_guid_files, args.dataset_name, args.train_init_summs):
            train_files = dict()
    ...
    dev_exs = []
    for dev_src, dev_src_tag, dev_tgt, dev_guid, dataset_name,dev_init_summ in \
            zip(args.dev_src_files, args.dev_src_tag_files,
                args.dev_tgt_files, args.dev_guid_files, args.dataset_name, args.dev_init_summs):
        dev_files = dict()
...
#     references[i] = [line]
#
# bleu, rouge_l, meteor, precision, recall, f1 = eval_accuracies(hypotheses,references,None,mode='dev')
# print(f"bleu={bleu}|rouge_l={rouge_l}|precision={precision}|recall={recall}|f1={f1}")