...
def add_train_args(parser):
    ...
    runtime.add_argument('--num_epochs', type=int, default=300,
                         help='Train data iterations')
    runtime.add_argument('--train_batch_size', type=int, default=16,
    ...
    files.add_argument('--model_name', type=str, default='sacs',
                       help='Unique model identifier (.mdl, .txt, .checkpoint)')
    files.add_argument('--data_dir', type=str, default='../../data/',
                       help='Directory of training/validation data')
    files.add_argument('--train_src', nargs='+', type=str, default=['test4.token.code'],
                       help='Preprocessed train source file')
    files.add_argument('--train_src_tag', nargs='+', type=str, default=[None],
                       help='Preprocessed train source tag file')
    files.add_argument('--train_tgt', nargs='+', type=str, default=['test.token.nl'],
                       help='Preprocessed train target file')
    files.add_argument('--dev_src', nargs='+', type=str, default=['valid4.token.code'],
                       help='Preprocessed dev source file')
    files.add_argument('--dev_src_tag', nargs='+', type=str,
                       help='Preprocessed dev source tag file')
    files.add_argument('--dev_tgt', nargs='+', type=str, default=['valid.token.nl'],
                       help='Preprocessed dev target file')
    files.add_argument('--train_guid', nargs='+', type=str, default=['test.token.guid'],
    ...
    files.add_argument('--train_init_summ', type='bool', default=['test1.init.summ'],
                       help='Use structural information of code')
    files.add_argument('--dev_init_summ', type='bool', default=['valid1.init.summ'],
                       help='Use structural information of code')
    files.add_argument('--train_src_code', type='bool', default=['test3.source.code'],
                       help='Use structural information of code')
    files.add_argument('--dev_src_code', type='bool', default=['valid3.source.code'],
                       help='Use structural information of code')
    files.add_argument('--train_map', type='bool', default=['test4.map'],
                       help='Use structural information of code')
    files.add_argument('--dev_map', type='bool', default=['valid4.map'],
                       help='Use structural information of code')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        args.train_init_summs = []
        args.train_src_codes = []
        args.train_maps = []
        ...
        if num_dataset > 1:
            ...
            if len(args.train_init_summ) == 1:
                args.train_init_summ = args.train_init_summ * num_dataset
            if len(args.train_src_code) == 1:
                args.train_src_code = args.train_src_code * num_dataset
            if len(args.train_map) == 1:
                args.train_map = args.train_map * num_dataset
        for i in range(num_dataset):
            ...
            train_init_summ = os.path.join(data_dir, args.train_init_summ[i])
            train_src_code = os.path.join(data_dir, args.train_src_code[i])
            train_map = os.path.join(data_dir, args.train_map[i])
            if not os.path.isfile(train_src_code):
                raise IOError('No such file: %s' % train_src_code)
            if not os.path.isfile(train_map):
                raise IOError('No such file: %s' % train_map)
            if not os.path.isfile(train_init_summ):
                raise IOError('No such file: %s' % train_init_summ)
            ...
            args.train_init_summs.append(train_init_summ)
            args.train_src_codes.append(train_src_code)
            args.train_maps.append(train_map)
    ...
    if num_dataset > 1:
        ...
        if len(args.dev_init_summ) == 1:
            args.dev_init_summ = args.dev_init_summ * num_dataset
        if len(args.dev_src_code) == 1:
            args.dev_src_code = args.dev_src_code * num_dataset
        if len(args.dev_map) == 1:
            args.dev_map = args.dev_map * num_dataset
    for i in range(num_dataset):
        dataset_name = args.dataset_name[i]
        data_dir = os.path.join(args.data_dir, dataset_name)
        dev_src = os.path.join(data_dir, args.dev_src[i])
        dev_tgt = os.path.join(data_dir, args.dev_tgt[i])
        dev_init_summ = os.path.join(data_dir, args.dev_init_summ[i])
        dev_src_code = os.path.join(data_dir, args.dev_src_code[i])
        dev_map = os.path.join(data_dir, args.dev_map[i])

        if not os.path.isfile(dev_src_code):
            raise IOError('No such file: %s' % dev_src_code)
        if not os.path.isfile(dev_map):
            raise IOError('No such file: %s' % dev_map)
        if not os.path.isfile(dev_init_summ):
            raise IOError('No such file: %s' % dev_init_summ)
        ...
        args.dev_init_summs.append(dev_init_summ)
        args.dev_src_codes.append(dev_src_code)
        args.dev_maps.append(dev_map)
...
def main(args):
    ...
    if not args.only_test:
        ...
        for train_src, train_src_tag, train_tgt, train_guid, dataset_name, train_init_summ, train_src_code, train_map in \
                zip(args.train_src_files, args.train_src_tag_files,
                    args.train_tgt_files, args.train_guid_files, args.dataset_name, args.train_init_summs, args.train_src_codes, args.train_maps):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tag'] = train_src_tag
            train_files['tgt'] = train_tgt
            train_files['guid'] = train_guid
            train_files['init_summ'] = train_init_summ
            train_files['src_code'] = train_src_code
            train_files['map'] = train_map
    ...
    dev_exs = []
    for dev_src, dev_src_tag, dev_tgt, dev_guid, dataset_name,dev_init_summ, dev_src_code, dev_map in \
            zip(args.dev_src_files, args.dev_src_tag_files,
                args.dev_tgt_files, args.dev_guid_files, args.dataset_name, args.dev_init_summs, args.dev_src_codes, args.dev_maps):
        dev_files = dict()
        dev_files['src'] = dev_src
        dev_files['src_tag'] = dev_src_tag
        dev_files['tgt'] = dev_tgt
        dev_files['guid'] = dev_guid
        dev_files['init_summ'] = dev_init_summ
        dev_files['src_code'] = dev_src_code
        dev_files['map'] = dev_map
        exs = util.load_data(args,
                             dev_files,
                             max_examples=args.max_examples,
                             dataset_name=dataset_name,
    ...
    # TRAIN/VALID LOOP
        ...
        if args.optimizer in ['sgd', 'adam'] and args.warmup_epochs >= start_epoch:
            ...
            num_batches = len(train_loader.dataset) // args.train_batch_size
