...
def add_test_args(parser):
    ...
    files.add_argument('--model_name', type=str, default='java_matrixsquare_ast4_SAGEConv_multihead_add_tok_cat_lv_emb_db_dec',
                       help='Unique model identifier (.mdl, .txt, .checkpoint)')
    files.add_argument('--data_dir', type=str, default='../../data/',
                       help='Directory of training/validation data')
    files.add_argument('--dev_src', nargs='+', type=str, default=['test4.token.code'],
    ...
    files.add_argument('--dev_init_summ', type='bool', default=['test1.init.summ'],
                       help='Use structural information of code')
    files.add_argument('--dev_src_code', type='bool', default=['test3.source.code'],
                       help='Use structural information of code')
    files.add_argument('--dev_map', type='bool', default=['test4.map'],
                       help='Use structural information of code')
...
def prepare_batch(batch, model):
    ...
    # code_level_rep = batch['code_level_rep']
    src_code_rep = batch['src_code_rep']
    src_code_len_rep = batch['src_code_len_rep']
    map = batch['map']
    edge_rep = batch['edge_rep']
    node_rep = batch['node_rep']
    node_batch_rep = batch['node_batch_rep']
    node_level_rep = batch['node_level_rep']
    ...
    code_level_rep = batch['code_level_rep']
    init_summ_rep = batch['init_summ_rep']
    init_summ_len_rep = batch['init_summ_len_rep']
    if args.cuda:
        ...
        if init_summ_rep is not None:
            init_summ_rep = init_summ_rep.cuda(non_blocking=True)
        if init_summ_len_rep is not None:
            init_summ_len_rep = init_summ_len_rep.cuda(non_blocking=True)
        if code_level_rep is not None:
            code_level_rep = code_level_rep.cuda(non_blocking=True)
        if src_code_rep is not None:
            src_code_rep = src_code_rep.cuda(non_blocking=True)
        if src_code_len_rep is not None:
            src_code_len_rep = src_code_len_rep.cuda(non_blocking=True)
        if node_rep is not None:
            node_rep = node_rep.cuda(non_blocking=True)
        if edge_rep is not None:
            edge_rep = edge_rep.cuda(non_blocking=True)
        if node_batch_rep is not None:
            node_batch_rep = node_batch_rep.cuda(non_blocking=True)
        if node_level_rep is not None:
            node_level_rep = node_level_rep.cuda(non_blocking=True)
    ...
    batch_inputs['init_summ_rep'] = init_summ_rep
    batch_inputs['init_summ_len_rep'] = init_summ_len_rep
    batch_inputs['code_level_rep'] = code_level_rep
    batch_inputs['src_code_rep'] = src_code_rep
    batch_inputs['src_code_len_rep'] = src_code_len_rep
    batch_inputs['map'] = map
    batch_inputs['node_rep'] = node_rep
    batch_inputs['edge_rep'] = edge_rep
    batch_inputs['node_batch_rep'] = node_batch_rep
    batch_inputs['node_level_rep'] = node_level_rep
    return batch_inputs
...
def validate_official(args, data_loader, model):
    ...
    if args.only_generate:
        ...
        bleu, rouge_l, meteor, precision, recall, f1, ind_bleu, ind_rouge, ind_meteor = \
        ...
        with open(args.pred_file, 'w') as fw:
            for eid, translation in trans_dict.items():
                ...
                out_dict['ind_meteor'] = ind_meteor[eid]
...
def eval_accuracies(hypotheses, references):
    ...
    return bleu * 100, rouge_l * 100, meteor * 100, precision.avg * 100, \
           recall.avg * 100, f1.avg * 100, ind_bleu, ind_rouge, ind_meteor
