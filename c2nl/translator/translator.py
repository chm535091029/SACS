...
class Translator(object):
    ...
    def translate_batch(self, batch_inputs):
        ...
        init_summ_rep = batch_inputs['init_summ_rep']
        code_level_rep = batch_inputs['code_level_rep']
        src_code_rep = batch_inputs['src_code_rep']
        src_code_len_rep = batch_inputs['src_code_len_rep']
        node_rep = batch_inputs['node_rep']
        edge_rep = batch_inputs['edge_rep']
        node_batch_rep = batch_inputs['node_batch_rep']
        node_level_rep = batch_inputs['node_level_rep']
        map = batch_inputs['map']
        init_summ_len_rep = batch_inputs['init_summ_len_rep']
        ...
        gnn_enc = gnn(node_rep,node_level_rep,edge_rep,node_batch_rep,code_word_rep.size(-1))
        src_code_rep = embedder(src_code_rep,
                                     None,
                                     None,
                                     None,
                                     None,
        ...
        memory_bank1 = gnn_enc
        # memory_bank: B x P x h; enc_states: l*num_directions x B x h
        if model_name == 'Transformer':
            memory_bank, layer_wise_outputs = encoder(code_rep, code_len, code_struc_rep,None,None)
            # memory_bank1, layer_wise_outputs1 = encoder(init_summ_emb, init_summ_len_rep, None)

            src_lens = code_len.repeat(beam_size)
            init_summ_len_rep = init_summ_len_rep.repeat(beam_size)
            dec_states = decoder.init_decoder(src_lens, memory_bank.shape[1],src_lens, memory_bank.shape[1])
        else:
