...
def build_input(ast):
    ...
    att_matrix = np.zeros((2,400), dtype=int)
    ...
    for edge in edges:
        try:
            if edge[0] <400 and edge[1]<400:
                att_matrix[0,i], att_matrix[1,i] = edge[0], edge[1]
                if edge[0]!=edge[1]:
                    att_matrix[1,i], att_matrix[0,i] = edge[1], edge[0]
        except:
            pass
...
for i, ast in tqdm(enumerate(asts)):
    ...
    np.savez_compressed(f"../../data/java/att_gat/{i}.npz", data=att_matrix.data, row=att_matrix.row, col=att_matrix.col,
