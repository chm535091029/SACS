...
def build_input(ast):
    ...
    att_matrix = np.zeros((512,512), dtype=bool)
    ...
    for node in ast:
        ...
        if 'children' in node.keys():
            ...
            node_map[str(node['id'])].append(len(ast_token))
    ...
    for node in ast:
        if "children" in node.keys():
            for child in node["children"]:
                for sub in node_map[str(child)]:
                    edges.add((node_map[str(node['id'])][0],sub))
        else:
            leave_id.extend(node_map[str(node['id'])])
    # 加一条自己到自己的边
        ...
        if "children" in node.keys():
            for child in node["children"]:
                ...
                if node["type"] == "ForStatement" and last_child is not None:
                    for sub in node_map[str(child)]:
                        try:
                            edges.add((node_map[str(last_child)][0],sub))
                        except:
                            pass
                    # edges.add((node_map[str(child)],node_map[str(last_child)]))
                    try:
                        for sub in node_map[str(next_siblings[str(node['id'])])]:
                            edges.add((node_map[str(node['id'])], sub))
                ...
                if node['type'] == 'SwitchStatement' and ast[child]['type'] == 'SwitchStatementCase' and str(node['id']) in next_siblings and 'children' in ast[child]:
                    for i in ast[child]['children']:
                        if 'BreakStatement' in ast[i]['type']:
                            ...
                            for sub in node_map[str(next_siblings[str(node['id'])])]:
                                try:
                                    edges.add((node_map[str(i)], sub))  # except:
                                except:
                                    pass
    ...
    for node in ast:
        ...
        if node["type"] == "FormalParameter":
            ...
            data_flow_graph[variable_name] = node_map[str(node["id"])][0]
        ...
        # 如果是变量使用节点，将其添加到对应变量的usages中
            ...
            if variable_name in data_flow_graph:
                for sub in node_map[str(node["id"])]:
                    edge1 = (data_flow_graph[variable_name],sub)
                    edges.add(edge1)
                # edges.add(edge2)
            #     last_use[variable_name] = node_map[str(node["id"])]
...
asts = f.readlines()
f1 = open("../../data/java/train1.token.code","w",encoding="utf-8")
f2 = open("../../data/java/valid1.token.code","w",encoding="utf-8")
f3 = open("../../data/java/test1.token.code","w",encoding="utf-8")
for i, ast in tqdm(enumerate(asts)):
    ast = json.loads(ast)
    ast_token,att_matrix = build_input(ast)
    # np.save(f"../../data/java/att/{i}.npy",att_matrix)
    np.savez_compressed(f"../../data/java/att1/{i}.npz", data=att_matrix.data, row=att_matrix.row, col=att_matrix.col,
