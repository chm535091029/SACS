    ...
    def __init__(self, head_count, model_dim, d_k, d_v, dropout=0.1,
        ...
        if max_relative_positions > 0:
            ...
            self.relative_positions_embeddings_v = nn.Embedding(
                vocab_size, self.d_v)
        self.head_parameter = nn.Parameter(torch.tensor([4, 2, 2, 2, 2, 2, 2], dtype=torch.float32))
    def forward(self, key, value, query, mask=None, layer_cache=None,
                attn_type=None, step=None, coverage=None, adj=None):
        """
        ...
        if adj is not None:
            global_edge = (adj > 0).int()
            original_edge = (adj == 1).int()
            control_edge = (adj == 2).int()
            dataflow_edge = (adj == 3).int()

            dataflow_edge_2 = torch.matmul((original_edge+dataflow_edge).float(),(original_edge+dataflow_edge).float().transpose(-1,-2))
            control_edge_2 = torch.matmul((original_edge+dataflow_edge).float(),(original_edge+control_edge).float().transpose(-1,-2))
            original_edge_2 = torch.matmul(original_edge.float(),original_edge.float().transpose(-1,-2))
            num_heads_per_type = F.softmax(self.head_parameter, dim=0) * self.head_count

            # 将数量四舍五入到最接近的整数
            num_heads_per_type = torch.round(num_heads_per_type).long()

            # 确保总数量等于总的注意力头数量
            num_heads_per_type[-1] = self.head_count - num_heads_per_type[:-1].sum()
            global_enhence_map = torch.stack([global_edge for i in range(num_heads_per_type[0])]+
                                             [original_edge for i in range(num_heads_per_type[1])]+
                                             [control_edge for i in range(num_heads_per_type[2])]+
                                             [dataflow_edge for i in range(num_heads_per_type[3])]+
                                             [dataflow_edge_2 for i in range(num_heads_per_type[4])]+
                                             [control_edge_2 for i in range(num_heads_per_type[5])]+
                                             [original_edge_2 for i in range(num_heads_per_type[6])]
                                             ,1)
            global_enhence_map = 5 * global_enhence_map.mul(scores)
            scores = scores + global_enhence_map
        if mask is not None:
