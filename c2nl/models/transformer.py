...
from c2nl.encoders.transformer import ASTEnc
# from ggnn import GGNN
# from dynamic_gat_encoder import Multi_GAT
from .gat import GAT
class Embedder(nn.Module):
    def __init__(self, args):
        ...
        self.enc_input_size += self.max_level
        self.dec_input_size += self.max_level
        ...
        # self.src_code_embeddings = Embeddings(self.enc_input_size,
        #                                       args.src_vocab_size,
        #                                       constants.PAD)
        # self.attention = MultiHeadedAttention(args.num_head,
        #                                       self.enc_input_size,
        #                                       args.d_k,
        #                                       args.d_v,
        #                                       dropout=args.trans_drop,
        #                                       max_relative_positions=args.max_relative_pos[0],
        #                                       use_neg_dist=args.use_neg_dist)
        # self.ffn = nn.Linear(self.enc_input_size,args.emsize)
        # self.src_code_embeddings = nn.Embedding(args.max_level,
        #                                     )
        if self.use_tgt_word:
            self.dec_input_size += args.emsize
            self.tgt_word_embeddings = Embeddings(self.dec_input_size,
                                                  args.tgt_vocab_size,
                                                  constants.PAD)
    ...
    def forward(self,
        ...
        if mode == 'encoder':
            word_rep = None
            if self.use_src_word:
                # if code_level_rep is None and map is None:
                #     word_rep = self.src_code_embeddings(sequence.unsqueeze(2))
                # else:
            ...
            if src_emb is not None and map is not None:
                for i, m in enumerate(map):
                    for ast_idx,src_idx in m.items():
                        try:
                            word_rep[i,int(ast_idx)] = src_emb[i,id] + word_rep[i,int(ast_idx)]
                        except:
                            pass
            if code_level_rep is not None:
                eye = torch.eye(self.max_level)
                if word_rep.is_cuda:
                    eye = eye.cuda()
                code_level_rep = eye[code_level_rep.squeeze(1)]
                word_rep = torch.cat((word_rep,code_level_rep),dim=-1)
...
class Encoder(nn.Module):
    ...
    def forward(self,
        ...
        layer_outputs, _ = self.transformer(input, input_len, adjacency,map,src_enc)  # B x seq_len x h
        if self.use_all_enc_layers:
...
class Decoder(nn.Module):
    ...
    def init_decoder(self,
        ...
        if self.split_decoder:
            ...
            return self.transformer.init_state(src_lens, max_src_len,src_lens1, max_src_len1) #
...
class Transformer(nn.Module):
    ...
    def __init__(self, args, tgt_dict):
        ...
        # args.max_level = args.max_level + args.
        self.encoder = Encoder(args,self.embedder.enc_input_size)
        self.gnn = ASTEnc(args.src_vocab_size,args.max_level,self.embedder.enc_input_size,self.embedder.enc_input_size,6)
        # self.encoder1 = Encoder(args, self.embedder.enc_input_size)
        # self.encoder = GAT(self.embedder.enc_input_size,64,self.embedder.enc_input_size,0.1,0.2,8)
        # self.gat_encoder = Multi_GAT(self.embedder.enc_input_size,self.embedder.enc_input_size,8,6)
        self.decoder = Decoder(args, self.embedder.enc_input_size)
    ...
    def _run_forward_ml(self,
        ...
        gnn_enc = self.gnn(node_rep,node_level_rep,edge_rep,node_batch_rep,code_word_rep.size(-1))
        src_code_rep = self.embedder(src_code_rep,
                                 None,
                                 None,
                                 None,
                                 None,
        ...
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len, code_struc_rep,None,None)
        # node_mask = ~sequence_mask(code_len, max_len=code_rep.size(1))
        # memory_bank = self.encoder(code_rep, code_struc_rep,node_mask)
        # memory_bank = self.gat_encoder(batched_g,code_rep)
        # B x seq_len x h, B x seq_len x layer x h,
        ...
        # enc_outputs1 = memory_bank1
        # enc_outputs = torch.cat((memory_bank,memory_bank1),dim=1)
        # mem_len = code_len+init_summ_len_rep
    ...
    def __generate_sequence(self,
        ...
        max_mem_len1 = params['memory_bank1'].shape[1]
        dec_states = self.decoder.init_decoder(params['src_len'], max_mem_len,params['src_len1'],max_mem_len1) #

        attns = {"coverage": None}
        enc_outputs = params['layer_wise_outputs'] if self.layer_wise_attn \
            else params['memory_bank']
        enc_outputs1 = params['memory_bank1']
        # +1 for <EOS> token
    ...
    def decode(self,
        ...
        # init_summ_emb = self.embedder(init_summ_rep,
        #                                  None,
        #                                  mode='encoder')
        # memory_bank1, layer_wise_outputs1 = self.encoder(init_summ_emb, init_summ_len_rep, None)



        gnn_enc = self.gnn(node_rep,node_level_rep,edge_rep,node_batch_rep,code_level_rep.size(-1))
        ...
        src_code_rep = self.embedder(src_code_rep,
                                 ...
                                 None,
                                 mode='encoder')
        # memory_bank1, layer_wise_outputs1 = self.encoder(src_code_rep, src_code_len_rep, None,None,None)
        ...
        code_rep = self.embedder(code_word_rep,
                                 ...
                                 mode='encoder')
        code_struc_rep = kwargs['code_struc_rep']
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len, code_struc_rep,None,None)        # node_mask = ~sequence_mask(code_len, max_len=word_rep.size(1))
        # memory_bank = self.encoder(word_rep, code_struc_rep,node_mask)
        # memory_bank, layer_wise_outputs = self.gat_encoder(batched_g, word_rep)

        # memory_bank = torch.cat((memory_bank,memory_bank1),dim=1)
        params = dict()
        params['memory_bank'] = memory_bank
        params['memory_bank1'] = gnn_enc
        params['layer_wise_outputs'] = layer_wise_outputs
        params['src_len'] = code_len
        params['src_len1'] = code_len
        # params['src_len'] = code_len + init_summ_len_rep
